...
export const FreescoutTeamDetails = ({ team }) => {
  const { resetTeamIntegration } = useSettings();
  const secret = useMemo(() => {
    if (!team || !team.hasOwnProperty("helpdesk")) {
      return null;
    }
    if (-1 != team.helpdesk.findIndex((helpdesk) => "freescout" === helpdesk)) {
      return team.helpdesk_settings["freescout"].secret;
    } else {
      return null;
    }
  }, [team]);
  const callback = useMemo(() => {
    ...
    if (-1 != team.helpdesk.findIndex((helpdesk) => "freescout" === helpdesk)) {
      return team.helpdesk_settings["freescout"].callback;
    } else {
      return null;
    }
  }, [team]);
    ...
    <div className="flex flex-1 flex-col space-y-6">
      <div>
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => {
              resetTeamIntegration(team.account_id, "freescout");
            }}
            className="flex items-center font-medium text-sm text-red-700"
            title="Warning: will issue new key and cannot be undone">
            <svg
...
export function ConfigureHelpscout({ isOpen, setIsOpen, team }) {
...
export function ConfigureFreescout({ isOpen, setIsOpen, team }) {
  let title = useMemo(() => __("Configure Free Scout"), []);
  return (
    <ConfigureIntegration
      isOpen={isOpen}
      setIsOpen={setIsOpen}
    ...
    >
      <>
        <FreescoutTeamDetails team={team} />
      </>
    </ConfigureIntegration>
  );
}
